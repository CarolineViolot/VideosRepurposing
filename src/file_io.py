...
def read_video_file(filename):
    if "youtube" in filename:
        ...
        videos_df = pd.read_json(filename)
...
def read_transcript_file(filepath):
    if "youtube" in filepath:
        ...
        transcripts.loc[transcripts['transcript'].isin(error_messages), "error"] = \
            transcripts.loc[transcripts['transcript'].isin(error_messages), "transcript"]
        transcripts.loc[transcripts['transcript'].isin(error_messages), "transcript"] = ""
        return transcripts
    elif "tiktok" in filepath:
        with open(filepath) as f:
            transcripts = pd.DataFrame(json.load(f))[['id', 'voice_to_text']].rename(columns={
                "id":"videoId", 'voice_to_text':'transcript'})
